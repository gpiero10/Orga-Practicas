...
void addlast(list_t* l, void* data){
    node_t* n = malloc(sizeof(node_t));
    switch(l->type) {
        case TypeFAT32:
        n->data = (void*) copy_fat32((fat32_t*) data);
    ...
    if (lista->size <= 1){
        ...
        if (n == 0) {
            ...
            if (nodo_n->next == nodo_m) {
                ...
                nodo_m->next = nodo_n;
                nodo_n->previous = nodo_m;
                lista->first = nodo_m;
                nodo_m->previous = NULL;
            } else {
                node_t *temp = nodo_m->next;
                lista->first = nodo_m;
                nodo_m->next = nodo_n->next;
                nodo_n->next->previous = nodo_m;

                anterior_m->next = nodo_n;
                nodo_n->next = temp;
                
                if (temp){
                    temp->previous = nodo_n;
                } else {
                    lista->last = nodo_n;
                }
